...
import WaterOptimizationDashboard from './components/WaterOptimizationDashboard';
import WaterOptimizationInsights from './components/WaterOptimizationInsights';
...
function StrategyCard({ strategy, onViewDetails, isSelected, onToggleSelect }) {
  ...
  return (
    <div className={`bg-white border-2 rounded-xl hover:shadow-lg transition-all duration-300 overflow-hidden group cursor-pointer ${
      isSelected ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:border-slate-300'
    }`} onClick={() => onToggleSelect && onToggleSelect(strategy.id)}>
      {/* Header Bar */}
      <div className="h-1 bg-gradient-to-r from-blue-600 to-blue-500"></div>
      
      <div className="p-6">
        {/* Title and Status - with checkbox */}
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-start gap-3 flex-1">
            <input 
              ...
              onChange={(e) => {
                e.stopPropagation();
                onToggleSelect && onToggleSelect(strategy.id);
              }}
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        ...
        <div className="px-8 border-t border-slate-200">
          <div className="flex gap-1 overflow-x-auto">
            {['overview', 'strategies', 'calculator', 'analytics', 'optimization', 'insights', 'admin'].map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-6 py-3 font-semibold border-b-2 transition-all text-sm uppercase tracking-wide whitespace-nowrap ${
                  activeTab === tab
                    ? 'border-blue-600 text-blue-600 bg-blue-50'
                    : 'border-transparent text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                }`}
              >
                {tab === 'admin' ? 'Control Panel' : tab === 'optimization' ? 'AI Optimizer' : tab === 'insights' ? 'Engine Insights' : tab.charAt(0).toUpperCase() + tab.slice(1)}
              </button>
            ))}
          </div>
        </div>
